...
export async function POST(_: NextRequest, { params }: { params: { id: string } }) {
  ...
  const campaign = await prisma.campaign.findFirst({
    ...
    include: { contactGroup: { include: { contacts: { where: { status: "PENDING" }, take: 10 } } } },
  });

  if (!campaign) return NextResponse.json({ error: "Campaign not found" }, { status: 404 });

  if (!campaign.contactGroupId) {
    return NextResponse.json({ error: "Campaign has no contact group assigned. Edit the campaign and select a contact group." }, { status: 400 });
  }
